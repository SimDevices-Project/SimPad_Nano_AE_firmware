...
sbit BT3 = P1^6;//默认设置Esc

BTB[6]={0,5,7,6};

#define BT(x) ((P1>>BTB[x])&1)
...
// unsigned char xdata keyCode4[] = {0x00,0x00,0x00};
// unsigned char xdata keyCode5[] = {0x00,0x00,0x00};
...
unsigned int data BTkeyState[6] = {0,0,0,0,0,0};

unsigned int data BTCoolDown[6] = {0,0,0,0,0,0};
...
UINT8C	ProdInfo[] = {
    // SimPad Nano Keypad
    0x26,0x03,0x53,0x00,0x69,0x00,0x6D,0x00,0x50,0x00,0x61,0x00,0x64,0x00,0x20,0x00,
    0x4E,0x00,0x61,0x00,0x6E,0x00,0x6F,0x00,0x20,0x00,0x4B,0x00,0x65,0x00,0x79,0x00,
    0x70,0x00,0x61,0x00,0x64,0x00
};
UINT8C ProdInfo_zh_CN[]=
{
    // SimPad Nano 迷你键盘
    0x22,0x03,0x53,0x00,0x69,0x00,0x6D,0x00,0x50,0x00,0x61,0x00,0x64,0x00,0x20,0x00,
    0x4E,0x00,0x61,0x00,0x6E,0x00,0x6F,0x00,0x20,0x00,0xF7,0x8F,0x60,0x4F,0x2E,0x95,
    0xD8,0x76
};
UINT8C ProdInfo_zh_TW_HK[]=
{
    // SimPad Nano 迷你鍵盤
    0x22,0x03,0x53,0x00,0x69,0x00,0x6D,0x00,0x50,0x00,0x61,0x00,0x64,0x00,0x20,0x00,
    0x4E,0x00,0x61,0x00,0x6E,0x00,0x6F,0x00,0x20,0x00,0xF7,0x8F,0x60,0x4F,0x75,0x93,
    0xE4,0x76
};
UINT8C ProdInfo_ja_JP[]=
{
    // SimPad Nano ミニキーボード
    0x28,0x03,0x53,0x00,0x69,0x00,0x6D,0x00,0x50,0x00,0x61,0x00,0x64,0x00,0x20,0x00,
    0x4E,0x00,0x61,0x00,0x6E,0x00,0x6F,0x00,0x20,0x00,0xDF,0x30,0xCB,0x30,0xAD,0x30,
        ...
        case UIS_TOKEN_OUT | 2:                                                 //endpoint 2# 端点批量下传
            if ( U_TOG_OK )                                                     // 不同步的数据包将丢弃
            {
                ...
                if(Ep2Buffer[0]) {
                    ...
                    // cmdCode[0] = Ep2Buffer[1];
                    // cmdCode[1] = Ep2Buffer[2];
                }
                getSms = 1;
            ...
            if(len == (sizeof(USB_SETUP_REQ)))
            {
                ...
                if ( ( UsbSetupBuf->bRequestType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )/* HID类命令 */
                {
                    ...
                    case 0x01://GetReport
                        ...
                        //if(SetupLen >= THIS_ENDP2_SIZE)                         //大于端点0大小，需要特殊处理
                        //{
                        //    len = THIS_ENDP2_SIZE;
                        //}
                        //else
...
void countEaseInOut() {
    ...
    easeOutInColorLevel0[3] = (unsigned int)colorR0 *2 >> 2;
    easeOutInColorLevel0[4] = (unsigned int)colorG0 *2 >> 2;
    easeOutInColorLevel0[5] = (unsigned int)colorB0 *2 >> 2;
    ...
    easeOutInColorLevel1[3] = (unsigned int)colorR1 *2 >> 2;
    easeOutInColorLevel1[4] = (unsigned int)colorG1 *2 >> 2;
    easeOutInColorLevel1[5] = (unsigned int)colorB1 *2 >> 2;
...
void countAndFlashLed() {
    ...
    case 0: {
        ...
        */
        ...
        else if(easeOutChangeMode && BTkeyState[1] == 0) {
            temp++;
        ...
        } else if(easeOutChangeMode2 && BTkeyState[2] == 0) {
            temp2++;
    ...
    case 5: {
        ...
        if(BTkeyState[1] && colorsChangeMode == 0) {
            colorsChangeMode = 1;
            colorChangeFlag = 1;
            colorToShowR0 = colorR0;
            colorToShowG0 = colorG0;
            colorToShowB0 = colorB0;
            isChangeColor = 0;
        } else if(colorsChangeMode && BTkeyState[1] == 0) {
            colorsChangeMode = 0;
            colorChangeFlag = 1;
            colorToShowR0 = 0;
            colorToShowG0 = 0;
        ...
        if(BTkeyState[2] && colorsChangeMode2 == 0) {
            colorsChangeMode2 = 1;
            colorChangeFlag = 1;
            colorToShowR1 = colorR1;
            colorToShowG1 = colorG1;
            colorToShowB1 = colorB1;
            isChangeColor = 0;
        } else if(colorsChangeMode2 && BTkeyState[2] == 0) {
            colorsChangeMode2 = 0;
            colorChangeFlag = 1;
            colorToShowR1 = 0;
            colorToShowG1 = 0;
...
void KeyScan() {
    ...
    for(i=1;i<=3;i++){
        ...
        if(BT(i) == 0 && BTkeyState[i] == 0) {
            KeyDownFunc(i);
            BTkeyState[i] = 1;
            BTCoolDown[i] = KEY_DELAY;
        }
        if(BT(i) != 0 && BTkeyState[i] == 1) {
            BTkeyState[i] = 0;
            BTCoolDown[i] = KEY_DELAY;
						UpKey();
        }
...
void KeyScanForLight() {
    if(BT(1) == 0) {
        BTCoolDown[1] = KEY_DELAY;
        BTkeyState[1] = 1;
    }
    if(BT(2) == 0) {
        BTCoolDown[2] = KEY_DELAY;
        BTkeyState[2] = 1;
    }
    return;
}
...
void refreshKeyCode() {
    ...
    // lightMode = 0x06;
    // 灯光模式额外配置
    if(lightMode == 0x06 || lightMode == 0x07) {
        // 彩虹模式
        // 这里当临时变量用而已
        // 找到颜色最浅的那个
        temp = colorR0;
    ...
    // ReadDataFlash(0x2C,3,BT_RollerkeyState);
		...
		if(LED_RAINBOW_DELAY == 0){ LED_RAINBOW_DELAY = LED_CHANGE_DELAY;}
		if(LED_EASE_DELAY == 0){ LED_EASE_DELAY = LED_CHANGE_DELAY;}
